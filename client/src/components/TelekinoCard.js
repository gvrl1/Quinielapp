...
import './TelekinoCard.css';

const TelekinoCard = ({ data, title, color }) => {
  if (!data || data.error) {
    return (
      <div className={`telekino-card ${color}`}>
              ...
              <div key={index} className="sorteo-item">
                ...
                <div className="numeros-container">
                  {sorteo.numeros && sorteo.numeros.map((numero, numIndex) => (
                    <div key={numIndex} className="numero-cuadrado">
                      {numero}
                    </div>
                  ))}
                </div>
